...
- Adding a note without adding its hub row in the same commit — the gap-close loop of
  [../guidelines/08-brownfield.md](../guidelines/08-brownfield.md) closes only when note, hub row,
  and tag entry have all landed.
- A hub reading like a phone book: past ~30 notes, split the folder — a hub that doesn't route
  is decoration.

## References

- [../guidelines/01-structure.md](../guidelines/01-structure.md) — the hub rule and folder anatomy.
- [../guidelines/04-validation.md](../guidelines/04-validation.md) — the missing-from-hub check.
- [document-template.md](document-template.md) — the note shape the hub routes to.
